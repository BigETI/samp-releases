/*
	SA:MP Soundz Player Filterscript erstellt von BigETI ©
	Ideen von FoxHound (http://forum.sa-mp.de/san-andreas-multiplayer/scripting-base/showroom/24082-sound-player-sopla/)
	Dieses Script ist keine Erweiterung oder Kopie des SoPla Filterscripts.
	Daher gilt dieses Script natürlich unter die Copyrights ©
*/
...
{
	print("\n======================================================");
	print(" SA:MP Soundz Player Filterscript erstellt von BigETI©");
	print("============= Alle Rechte gehören ihm! ===============");
	print("======================================================\n");
	return 1;
}
...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	{
	    ShowPlayerDialog(playerid, BSP_MENU_DIALOGID, DIALOG_STYLE_LIST, "SA:MP Soundz von BigETI", "Sounds (Alle)\nSuchen\nCredits\nBeenden", "Wählen", "Beenden");
		return 1;
	}
	return 0;
}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch(dialogid)
	{
				...
				switch(listitem)
				{
					case 0:
					{
						...
						for(new i = 0; i < MAX_ENTRIES; i++)
						{
							...
							else strcat(str, "Weiter");
						}
						ShowPlayerDialog(playerid, BSP_LIST1_DIALOGID, DIALOG_STYLE_LIST, "Alle Sounds (1/3)", str, "Wählen", "Zurück");
					}
					case 1:
					{
						SendClientMessage(playerid, RED, "Die Suchfunktion wurde deaktiviert. Warte bitte auf Version 2, um die Suchfunktion zu benutzen.");
						ShowPlayerDialog(playerid, BSP_MENU_DIALOGID, DIALOG_STYLE_LIST, "SA:MP Soundz von BigETI", "Sounds (Alle)\nSuchen\nCredits\nBeenden", "Wählen", "Beenden");
					}
					case 2: ShowPlayerDialog(playerid, BSP_CREDITS_DIALOGID, DIALOG_STYLE_MSGBOX, "Credits", "SA:MP Soundz V1 Filterscript erstellt\nvon BigETI.\nAlle Rechte gehören ihm!", "Zurück", "");
				}
			}
		}
		case BSP_LIST1_DIALOGID:
		{
			if(response)
			{
				if(listitem == MAX_ENTRIES-1)
				{
					...
					ShowPlayerDialog(playerid, BSP_LIST2_DIALOGID, DIALOG_STYLE_LIST, "Alle Sounds (2/3)", str, "Wählen", "Zurück");
				}
				else
				{
			...
			else ShowPlayerDialog(playerid, BSP_MENU_DIALOGID, DIALOG_STYLE_LIST, "SA:MP Soundz von BigETI", "Sounds (Alle)\nSuchen\nCredits\nBeenden", "Wählen", "Beenden");
		}
		case BSP_LIST2_DIALOGID:
		{
			if(response)
			{
				if(listitem == MAX_ENTRIES-1)
				{
					...
					ShowPlayerDialog(playerid, BSP_LIST3_DIALOGID, DIALOG_STYLE_LIST, "Alle Sounds (3/3)", str, "Wählen", "Zurück");
				}
				else
				{
		...
		case BSP_CREDITS_DIALOGID: ShowPlayerDialog(playerid, BSP_MENU_DIALOGID, DIALOG_STYLE_LIST, "SA:MP Soundz von BigETI", "Sounds (Alle)\nSuchen\nCredits\nBeenden", "Wählen", "Beenden");
	}
	return 1;
}